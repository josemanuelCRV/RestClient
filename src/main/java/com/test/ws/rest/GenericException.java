...
public class GenericException implements javax.ws.rs.ext.ExceptionMapper<Exception> {
	...
	public Response toResponse(Exception error) {
		...
		if (error instanceof WebApplicationException) {
			...
			return Response.status(webEx.getResponse().getStatus()).entity(json).type(MediaType.APPLICATION_JSON)
					.build();
		}
		response = Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity("Internal error").type("text/plain")
				.build();
		return response;
	}
}